...
state.periods = [
  { ...state.periods[0], id: "2026-06-h2", lockedBase: true },
  {
    id: "2026-07-h1",
    month: "Julio",
    label: "1a julio",
    note: "Sueldo recibido el 30 de junio.",
    salary: 8000,
    extraIncome: 250,
    partnerIncome: 0,
    rent: -1750,
...
const result = closePeriodFor(state, "2026-07-h1", "2026-06-30");
assert.equal(result.state.settings.currentSavings, 6600);
assert.equal(result.state.settings.rentReserve, 1750);
assert.equal(result.state.periods.find((period) => period.id === "2026-07-h1")?.salary, 0);
assert.equal(result.state.periods.find((period) => period.id === "2026-07-h1")?.closedAt, "2026-06-30");
assert.equal(result.state.periods.at(-1)?.id, "2026-08-h1");
assert.equal(result.state.periods.at(-1)?.salary, 8000);

const second = closePeriodFor(result.state, "2026-07-h1", "2026-06-30");
